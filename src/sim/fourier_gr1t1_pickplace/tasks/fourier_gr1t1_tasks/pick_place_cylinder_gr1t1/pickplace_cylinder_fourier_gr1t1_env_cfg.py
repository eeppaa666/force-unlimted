...
from tasks.common_config import FourierRobotPresets, CameraPresets  # isort: skip
...
class ObjectTableSceneCfg(TableCylinderSceneCfg):
    ...
    inherits from TableCylinderSceneCfg, gets the complete Fourier GR1T1 robot scene configuration
    can add task-specific scene elements or override default configurations here
    """
    
    # Fourier humanoid robot w/ arms higher
    # 5. humanoid robot configuration 
    robot: ArticulationCfg = FourierRobotPresets.fourier_gr1t1_base_fix()


    # camera tbd
    # 6. add camera configuration (Fourier GR1T1 specific)
    front_camera = CameraPresets.fourier_front_camera()
    left_wrist_camera = CameraPresets.fourier_left_wrist_camera()
    right_wrist_camera = CameraPresets.fourier_right_wrist_camera()
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        robot_gripper_state = ObsTerm(func=mdp.get_robot_gripper_joint_states)
        ...
        def __post_init__(self):
            ...
            self.concatenate_terms = False  # disable observation item connection (keep multi-modal observations separate)
...
@configclass
class PickPlaceFourierGR1T1BaseFixEnvCfg(ManagerBasedRLEnvCfg):
    """
    Fourier GR1T1 pick-place environment configuration
