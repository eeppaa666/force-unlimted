...
def _get_rewards_dds_instance():
    """get the DDS instance, delay initialization (optional)"""
    global _rewards_dds, _dds_initialized
    
    if not _dds_initialized or _rewards_dds is None:
        try:
            # dynamically import the DDS module (optional, for teleoperation)
            ...
            # DDS is optional, don't fail if not available
            print(f"[Observations Rewards] DDS communication not available (optional): {e}")
            _rewards_dds = None
