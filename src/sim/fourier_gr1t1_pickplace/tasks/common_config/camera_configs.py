...
class CameraPresets:
    ...
    # Fourier GR1T1 robot camera presets
    @classmethod
    def fourier_front_camera(cls) -> CameraCfg:
        """Fourier GR1T1 front camera configuration"""
        return CameraBaseCfg.get_camera_config(
            # Mount the camera as a child prim to avoid colliding with the existing head link prim.
            prim_path="/World/envs/env_.*/Robot/head_pitch_link/front_cam",
            height=480,
            width=640,
            update_period=0.02,
            data_types=["rgb"],
            ...
            horizontal_aperture=20.0,
            clipping_range=(0.1, 1.0e5),
            pos_offset=(0.03, 0.0, 0.05),  # Fourier head front offset
            rot_offset=(0.7071, 0, 0, 0.7071),  # facing forward
        )
    
    @classmethod
    def fourier_left_wrist_camera(cls) -> CameraCfg:
        """Fourier GR1T1 left wrist camera configuration"""
        return CameraBaseCfg.get_camera_config(
            prim_path="/World/envs/env_.*/Robot/L_hand_base_link",
            height=480,
            width=640,
            update_period=0.02,
            data_types=["rgb"],
            ...
            horizontal_aperture=20.0,
            clipping_range=(0.1, 1.0e5),
            pos_offset=(0.02, 0.045, 0.1),  # Fourier left hand offset
            rot_offset=(-0.34202, 0.93969, 0, 0),  # looking at hand
        )
    
    @classmethod
    def fourier_right_wrist_camera(cls) -> CameraCfg:
        """Fourier GR1T1 right wrist camera configuration"""
        return CameraBaseCfg.get_camera_config(
            prim_path="/World/envs/env_.*/Robot/R_hand_base_link",
            height=480,
            width=640,
            update_period=0.02,
            data_types=["rgb"],
            ...
            horizontal_aperture=20.0,
            clipping_range=(0.1, 1.0e5),
            pos_offset=(-0.02, 0.045, 0.1),  # Fourier right hand offset
            rot_offset=(-0.34202, 0.93969, 0, 0),  # looking at hand
        ) 